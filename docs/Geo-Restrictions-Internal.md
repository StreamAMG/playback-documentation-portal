...
- **Enable Article Access Management** is: **enabled**

The other items are not mandatory to be enabled.

---

## Feature Toggles

There are two feature toggles for this piece of work. Please note that these may no longer be applicable at the time of reading, but as of **October 2025**, they still are.

### CloudMatrix

If the following configuration **Enable Article Access Management** is set to **false**, then the Playback SNS Datastore will not send any of the new GeoRestriction data when a datastore item is being saved.

### Playback

If the following configuration is **false** or **undefined**, then no GEO Restriction data will be used when the Playback SDK plays a video.
...
This is a feature flag that is set on each playback configuration (per client). To change the value, use a PATCH request (see the Playback Configuration API PATCH documentation).

#### Feature Toggle Combinations

- CloudMatrix false & Playback Config false → No new Geo Restrictions

- CloudMatrix false & Playback Config true → No new Geo Restrictions

- CloudMatrix true & Playback Config false → No new Geo Restrictions

- CloudMatrix true & Playback Config true → New Geo Restrictions


## CloudMatrix Setup and Playback Logic

CloudMatrix functions exactly as it has before, but with some additional logic under the hood that is not exposed to Playback (i.e. all the logic is internal). Please follow the same setup process you used previously to configure a Geo Restriction.
